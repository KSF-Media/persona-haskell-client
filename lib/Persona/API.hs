...
type PersonaAPI
    ...
    :<|> "admin" :> "free-pass" :> ReqBody '[JSON] FreePassInput :> Header "AuthUser" UUID :> Header "Authorization" Text :> Verb 'POST 200 '[JSON] Text -- 'adminFreePassPost' route
...
data PersonaBackend m = PersonaBackend
  ...
  , adminFreePassDelete :: Text -> Maybe UUID -> Maybe Text -> m (){- ^ Revoking a free pass does not perform a hard delete, but rather marks the free pass with the given hash as being revoked in the DB. This is the only sensible way of deleting free passes as this kind of state information cannot be included in hashes. Revoked free passes can be reinstated with a simple SQL query. -}
  , adminFreePassPost :: FreePassInput -> Maybe UUID -> Maybe Text -> m Text{- ^ Free passes can be used to temporarily bypass the paywall for individual articles. This endpoint is stateless (and thus deterministic). -}
...
createPersonaClient = PersonaBackend{..}
  where
    ((coerce -> accountPasswordForgotGet) :<|>
     ...
     (coerce -> entitlementsFreePassGet) :<|>
